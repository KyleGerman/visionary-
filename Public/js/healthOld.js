window.addEventListener('DOMContentLoaded', async () => {
  ...
  async function load() {
    try {
      ...
      medHistoryEl.value = data.medHistory || '';
      medsEl.value = data.prescriptions || ''; // this needs to be a separate function I think, needs to be 'for each object'
    } catch (err) { console.error(err); }
  }

  document.getElementById('healthForm')?.addEventListener('submit', async (e) => { // this needs updating, run through /health api instead!
    e.preventDefault();
